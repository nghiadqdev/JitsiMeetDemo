...
const VideoLayout = {
    ...
    onPermissionDrawChange(id, isDraw) {
        if (APP.conference.isLocalId(id)) {
            localVideoThumbnail.showDrawIndicator(isDraw);
        } else {
            const remoteVideo = remoteVideos[id];
            ...
            remoteVideo.showDrawIndicator(isDraw);
        }
    },
    ...
    updateCamUser(id) { 
        let streamWithId;
        if(id == APP.conference.getMyUserId()){
            streamWithId = localVideoThumbnail ? localVideoThumbnail.videoStream : '';
        }else{
            streamWithId = remoteVideos[id] ? remoteVideos[id].videoStream : '';
        }
        if(!streamWithId){
            return;
        }
        let ElementVideo = document.createElement('video');
        ElementVideo.classList.add("video-" + id);
        const isVideo = streamWithId.isVideoTrack();
        if (isVideo) {
            ElementVideo.setAttribute('muted', 'true');
        }
        ElementVideo.autoplay = true;
       
        ElementVideo.id = "videoParticipant_"+streamWithId.getId();
        streamWithId.attach(ElementVideo);
        return ElementVideo;
    },

    updateLargeVideo(id, forceUpdate) {
        let participants = APP.store.getState()['features/base/participants'];
        
        let listTeacher = participants.filter(function (teacher) {
        ...
        if(listTeacher.length > 0 ){
            id = listTeacher[0].id;
            let _name = listTeacher[0].name
            if (listTeacher[0].name && listTeacher[0].name.split("-").length === 3) {
                const arr = listTeacher[0].name.split("-")
               // _name = `${arr[0]}-${arr[2]}-${arr[1]}`  <Prefix>-<ID>-<Name>
                _name = `${arr[1]}-${arr[2]}`
            }
            $('#teacherName').text(_name);
