...
function connect(id, password, roomName) {
    ...
    connectionConfig.serviceUrl += `?room=${roomName}`;

    let connection_jwt = undefined;
    let connection_issuer = undefined;
    let already_stored = false;
    if(!jwt){

        connection_jwt = jitsiLocalStorage.getItem('token-jwt');
        connection_issuer = jitsiLocalStorage.getItem('token-issuer');
        console.log("get JWT from storage =["+ connection_jwt + "]" + " by " + connection_issuer );//SON
        already_stored = true;
    }else{
        connection_jwt = jwt;
        connection_issuer = issuer;
    }
/*
    if( connectionConfig.useJWT && jwt && issuer && (issuer !== 'anonymous')){
        connection_jwt =jwt;
    }*/
    if( (!connectionConfig.useJWT) || (!connection_issuer) || (connection_issuer === 'anonymous')){
        connection_jwt = undefined;
    }

    console.log("Call JitsiConnection with JWT=["+connection_jwt + "] issuer ="+ connection_issuer  );//SON
    if(!already_stored && typeof(connection_jwt) !== 'undefined' && connection_jwt != null){
        console.log("Save to storage...");
        jitsiLocalStorage.setItem('token-jwt',  connection_jwt );
        jitsiLocalStorage.setItem('token-issuer', connection_issuer);
        console.log("Saving terminated!");
    }
