...
import {ROLE_STUDENT} from '../../../react/vcrx/base';
...
function RemoteVideo(user, VideoLayout, emitter) {
    ...
    this.addRemoteVideoContainer(user);
...
RemoteVideo.prototype.updateView = function() {
    const {isolated} = APP.store.getState()['emg'].roomInfo;
    const { role } = APP.store.getState()['emg'].userInfo;
    if (isolated == 1 && role == ROLE_STUDENT){
        $(`#participant_${this.id}`).css('display', 'none')
    }
...
RemoteVideo.createContainer = function(spanId) {
    ...
    container.innerHTML = `
        ...
        <div class = 'videocontainer__ban ban__chat'>
            <img src="../../images/emg/show-ban-chat.png"/>
        </div>
        <div class = 'videocontainer__ban ban__cam'>
            <img src="../../images/emg/ban-video-show.png"/>
        </div>
