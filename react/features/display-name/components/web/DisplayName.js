...
class DisplayName extends Component<Props, State> {
    ...
    render() {
        ...
        let _name = _nameToDisplay
        if (_nameToDisplay && _nameToDisplay.split("-").length === 3) {
            const arr = _nameToDisplay.split("-")
            //_nameToDisplay = <RoleName>-<UserName>-<UserID>
   //         _name = `${arr[0]}-${arr[2]}-${arr[1]}`
            _name = `${arr[1]}-${arr[2]}`
        }
        ...
        return (
            <span
                ...
                { appendSuffix(_name, displayNameSuffix) }
            </span>
        );
    }
