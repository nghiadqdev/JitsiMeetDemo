...
import { saveLogChat } from '/vcrx/base/apis';
        ...
        case SEND_MESSAGE: {
            ...
            if (conference) {
                ...
                if (shouldSendPrivateMessageTo) {
                    ...
                    const { userList } = state['emg'];
                    const idChat = getRandomInt()
                    const localId = APP.conference.getMyUserId()
                    const user = userList.find(({id})=>id == localId)
                    const isBanChat = user ? user.banChat : false
                    if (privateMessageRecipient) {
                        // saveLogChat({roomId: room, msg: action.message, id: idChat, isBanChat})
                        conference.sendPrivateTextMessage(privateMessageRecipient.id, JSON.stringify({msg : action.message, id :idChat, isBanChat}));
                        _persistSentPrivateMessage(store, privateMessageRecipient.id, JSON.stringify({msg : action.message, id :idChat, isBanChat}));
                    } else {
                        saveLogChat({roomId: room, msg: action.message, id: idChat, isBanChat})
                        conference.sendTextMessage(JSON.stringify({msg: action.message, id: idChat, isBanChat}));
                    }
                }
            }
            break;
...
function _handleReceivedMessage({ dispatch, getState }, { id, message, nick, privateMessage, timestamp }) {
    ...
    const msg = data.msg ? data.msg : message;
    const chatId = data.id ? data.id : null;
    const isBanChat = data.isBanChat ? data.isBanChat : null;
    if (!isBanChat 
        || (isBanChat && config.permission.receiveChatWhenBaned.indexOf(userInfo.role) != -1)
        || (isBanChat && localParticipant.id == id)
        ) {
        dispatch(addMessage({
            displayName,
            hasRead,
...
function _persistSentPrivateMessage({ dispatch, getState }, recipientID, message) {
    ...
    const msg = JSON.parse(message).msg ? JSON.parse(message).msg : message;
    const chatId = JSON.parse(message).msg ? JSON.parse(message).id : null;
    dispatch(addMessage({
        displayName,
        hasRead: true,
