...
import { 
    initConference,
    ROLE_KEY_TEACHER
} from './react/vcrx/base';
import { sendCommandVcrx } from './react/vcrx/sendCommandVcrx';
import { addVcrxListeners } from './react/vcrx/actionListenerVcrx';
...
export default {
    ...
    init(options) {
        ...
        let searchURL = APP.store.getState()['features/base/connection'].locationURL.searchParams;
        if (searchURL) {
            let token = searchURL.get('token');
            if (token) {
                this.paramUrl = token;
            }
    ...
    _getConferenceOptions() {
        ...
        const objectConfig = Object.assign({}, config);
        if(objectConfig.useJWT){
            options.openBridgeChannel = 'websocket'; //SON
            console.log("EMG: use websocket for Bridge Channel");
        }
    ...
    */
    ...
    sendCommandVcrxRoom(command,value, attribute){
        if (!room) {
            return;
        }
        sendCommandVcrx(room,command,value, attribute);
    }
};
