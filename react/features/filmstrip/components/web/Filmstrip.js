...
class Filmstrip extends Component <Props> {
    ...
    render() {
        ...
        const { _largeView, _tileViewEnabled, _role } = this.props;
        let hidden
        if (_largeView == '1') {
            hidden = FILMSTRIP.enableLocalCameraLargeView;
        } else {
            hidden = FILMSTRIP.enableLocalCamera;
        }
        ...
        return (
            <div className = { `filmstrip ${this.props._className}` }>
                ...
                <div
                    ...
                    id = 'remoteVideos'>
                    <div
                        className = {`filmstrip__videos ${_tileViewEnabled ? 'is-scroll-x' : 'is-scroll-y'}`}
                        ...
                        {/*
                          ...
                          */}
                        <div
                            className = {`remote-videos-container ${this.props._tileViewEnabled ? 'isMosaicMode': 'is-max-content'}`} 
                            ...
                            className = {`${hidden.includes(_role) ? 'hiddenElement' : ''}`} />
                        </div>
                    </div>
...
function _mapStateToProps(state) {
    ...
    const videosClassName = `filmstrip__videos${
        ...
        visible ? '' : ' hidden'}`;
    let { isolated, largeView } = state['emg'].roomInfo;
    let { role } = state['emg'].userInfo;
    return {
