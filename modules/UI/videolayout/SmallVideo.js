...
import {VIEW_LARGE_SMALL_CAM} from '../../../react/vcrx/base';
import {changeRoomInfo} from '../../../react/vcrx/actions';
import {StarRartingButton} from '../../../react/vcrx/conference/components';
import { updateLocalCamera, updateRemoteCamera } from '../../../react/vcrx/actions';
import {socket }                from '../../../react/vcrx/config';
...
function SmallVideo(VideoLayout) {
    ...
    //Tungns : add check draw permission
    this.isDraw = false
...
//Tungns add function showDrawIndicator
SmallVideo.prototype.showDrawIndicator = function (isDraw) {
...
SmallVideo.prototype.updateStatusBar = function () {
    ...
    let draw = APP.store.getState()['emg'].userList.find(({id}) => id == this.id)
    ReactDOM.render(
            ...
            <div>
                ...
                {(!draw || (draw && !draw.draw))
                    ...
                    <div className ='moderator-icon right'>
                        <div className = 'indicator-icon-container toolbar-icon'>
                            <div>
                                <img src={"../../images/emg/pencil-disable.png"} height ='50%' />
                            </div>
                        </div>
                    </div>
                    : null}
...
SmallVideo.prototype.updateView = function () {
    ...
    APP.conference.isLocalId(this.id) 
        ? APP.store.dispatch(updateLocalCamera(this.id))
        : APP.store.dispatch(updateRemoteCamera(this.id))
...
SmallVideo.prototype._onContainerClick = function (event) {
    const { roomInfo, userInfo } = APP.store.getState()['emg'];
    const triggerPin = this._shouldTriggerPin(event);

    setTimeout(() => {
        if(triggerPin){
            if (roomInfo.onState) {
                if(config.permission.onState.includes(userInfo.role)) {
                    const room = APP.store.getState()['features/base/conference'].room;
                    socket.emit("set-on-state-id", {
                        onStateId: this.id,
                        room
                    })
                }
                ...
                if (VIEW_LARGE_SMALL_CAM.includes(userInfo.role) && !$(".star-div").hasClass("not-largeCam")){
                    const tileViewEnabled = APP.store.getState()['features/video-layout'].tileViewEnabled;
                    tileViewEnabled && APP.store.dispatch(changeRoomInfo({
                        largeSmallCam : true,
                        largeSmallCamId: this.id
                    }))
                }
            }
        }
